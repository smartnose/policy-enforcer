...
from policy_enforcer.sk_agents import create_sk_agent
...
║         Semantic Kernel ReAct Agent {mode_text:<20} ║
║                   Powered by Google AI                    ║
╚═══════════════════════════════════════════════════════════╝

This demo showcases a custom ReAct agent built with Semantic Kernel
that enforces business rules when helping users choose activities.
...
Examples:
  python main_sk.py                    # Standard mode with rules
  python main_sk.py --no-rules         # Learning mode without upfront rules
  python main_sk.py --rules            # Explicitly enable rules mode
        """
    )
...
def main():
    ...
    # Create agent
    print("🚀 Initializing Semantic Kernel ReAct agent...")
    try:
